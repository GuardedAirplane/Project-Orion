...
bool dGPS::update(double desLat, double desLon){
    ...
    if (byteGPS==13){            // If the received byte is = to 13, end of transmission 
      ...
      if(bien==6){               // If yes, continue and process the data
        ...
        Serial.println(linea);   //Print out the GPRMC string (just for reference)
          ...
          double t=((desLon-fLon)*22)/1260; //difference between the longitudes in radians

          double cdist=acos(cos(((90-desLat)*22)/1260)*cos(((90-fLat)*22)/1260)+sin(((90-desLat)*22)/1260)*sin(((90-fLat)*22)/1260)*cos(t)); //angular distance
          fdist=6378.1*cdist; //distance= radius of earth * angular distance
          
          //------------------------------------------
          //Azimuth of the destination from current location
          double cazimuth=asin((sin(((90-desLat)*22)/1260)*sin(t))/sin(cdist)); //temporary azimuth(in radians)
...
double dGPS::conv_coords(double in_coords)
{
